document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to generate a consistent hash-based ID
  // converts your input into a SHA-256 hash
  async function generateShortHash(input) {
    ...
    const hashHex = hashArray
      ...
      .join("");
    return hashHex.substring(0, 8); // Shorten the hash to 8 characters
  }

  // Result
  // The form submission is handled asynchronously, using await to ensure the generateHash function completes before proceeding.
  $("#imgurForm").on("submit", async function (event) {
    ...
    var generatedId = await generateShortHash(imgurUrls);

    // Update the URL by appending the ID and the imgurUrls as query parameters
    var requestUrl = `controller.php?id=${generatedId}&imgurUrls=${encodeURIComponent(
      imgurUrls
    )}`;
  ...
  // Function to submit the input URLs
  async function submitImgurUrls(imgurUrls) {
    // Clear textareas before making the request
    $("#result").val("");
    $("#error_result").val("");
    $("#videoContainer").empty();
    ...
    // Generate a consistent hash-based ID
    const generatedId = await generateShortHash(imgurUrls.join("\n"));

    // Update the URL by appending the ID and the imgurUrls as query parameters
    const requestUrl = `controller.php?id=${generatedId}&imgurUrls=${encodeURIComponent(
      imgurUrls.join("\n")
    )}`;
  ...
  // Check the URL for the ID parameter on page load
  ...
  // The IntersectionObserver is now correctly observing the topBtnGroup inside the resultWrapper once the resultWrapper becomes visible.
  // The MutationObserver listens for changes to the style attribute of the resultWrapper,
  // so it can start or stop observing the topBtnGroup when the resultWrapper becomes visible or hidden.
